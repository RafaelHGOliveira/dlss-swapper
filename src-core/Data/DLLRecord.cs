...
{
    // Platform delegates — src/ sets these on startup
    public static Action<Action>? RunOnUIThreadDelegate;
    public static Action<ZipArchive, DLLRecord>? ExtractFromZipDelegate;
    public static Func<GameAssetType, string>? GetDownloadErrorMessageDelegate;
    public static Func<object?>? CreateTranslationPropertiesDelegate;
    ...
    [JsonIgnore]
    public object? TranslationProperties { get; private set; }

    public DLLRecord()
    {
        TranslationProperties = CreateTranslationPropertiesDelegate?.Invoke();
    }
    ...
    {
        (RunOnUIThreadDelegate ?? (a => a()))(action);
    }
    ...
    internal async Task<(bool Success, string Message, bool Cancelled)> DownloadAsync()
    {
        ...
        var tempZipFile = System.IO.Path.Combine(Storage.GetTemp(), $"{fileDownloader.Guid.ToString("D").ToUpper()}.zip");
        ...
        try
        {
            ...
            using (var fileStream = new FileStream(tempZipFile, FileMode.Create, FileAccess.ReadWrite, FileShare.None, FileDownloader.BufferSize, true))
            {
                ...
                {
                    ExtractFromZipDelegate?.Invoke(zipArchive, this);
                }
            }
        ...
        catch (Exception err)
        {
            ...
            RunOnUIThread(() =>
            {
                ...
                LocalRecord.DownloadErrorMessage = (GetDownloadErrorMessageDelegate?.Invoke(AssetType)) ?? AssetType.ToString();
                NotifyPropertyChanged(nameof(LocalRecord));
            });
